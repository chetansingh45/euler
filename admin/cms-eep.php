...
$sectionHero = $db->db("SELECT * FROM eep_page WHERE section_name = 'hero'");
...
if (isset($_POST['save_hero_section'])) {
    ...
    if (mysqli_num_rows($sectionHero) > 0) {
        $res = $db->db("UPDATE eep_page SET main_image='$main_image' WHERE section_name='hero'");
        if ($res) {
            alert("Updated", "Hero Section Updated Successfully", "success");
        }
    } else {
        $res = $db->db("INSERT INTO eep_page (section_name,main_image)
                VALUES ('hero','$main_image')");
        if ($res) {
...
$heroData = false;
$hero = $db->db("SELECT * FROM eep_page WHERE section_name = 'hero'");
...
$section_two = $db->db("SELECT * FROM eep_page WHERE section_name = 'section_two' AND main_title IS NOT NULL");
...
if (isset($_POST['save_section_two'])) {
    ...
    if (mysqli_num_rows($section_two) > 0) {
        $res = $db->db("UPDATE eep_page SET main_title='$main_title' WHERE section_name='section_two' AND main_title IS NOT NULL ");
        if ($res) {
            alert("Updated", "Section Two Updated Successfully", "success");
        }
    } else {
        $sql = "INSERT INTO eep_page (section_name,main_title) VALUES ('section_two','$main_title')";
...
if (isset($_POST['save_section_two_lines'])) {
    ...
    $sql = "INSERT INTO eep_page (section_name,title) VALUES ('section_two','$title')";
    $query = $db->db($sql);
    if ($query) {
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "DELETE FROM eep_page WHERE id='$id'";
    $query = $db->db($sql);
    if ($query) {
        redirect('cms-eep.php');
...
$section_meta = $db->db("SELECT * FROM eep_page WHERE section_name = 'meta'");
if (isset($_POST['save_meta'])) {
    $main_title = filter($_POST['main_title']);
    $main_description = filter($_POST['main_description']);
    if (mysqli_num_rows($section_meta) > 0) {
        $res = $db->db("UPDATE eep_page SET main_title='$main_title',main_description='$main_description' WHERE section_name='meta'");
        if ($res) {
            alert("Updated", "Meta Updated Successfully", "success");
        }
    } else {
        $sql = "INSERT INTO eep_page (section_name,main_title,main_description)
...
if (isset($_POST['save_section_three'])) {
    $section_three = $db->db("SELECT * FROM eep_page WHERE section_name = 'section_three' AND main_title IS NOT NULL");
    $section_three_data = mysqli_fetch_assoc($section_three);
    $main_title = filter($_POST['main_title']);
    if (mysqli_num_rows($section_three) > 0) {
        $res = $db->db("UPDATE eep_page SET main_title='$main_title' WHERE section_name='section_three' AND main_title IS NOT NULL ");
        if ($res) {
            alert("Updated", "Section Three Updated Successfully", "success");
        }
    } else {
        $sql = "INSERT INTO eep_page (section_name,main_title)
        VALUES ('section_three','$main_title')";
...
if (isset($_POST['save_section_three_lines'])) {
    ...
    $logo = $_POST['logo'];
    $sql = "INSERT INTO eep_page (section_name,title,logo,logo_alt) VALUES ('section_three','$title','$logo','$logo_alt')";
    $query = $db->db($sql);
    if ($query) {
        alert("Added", "New title added", "success");
    }
...
<div class="d-flex align-items-center mb-4 flex-wrap">
    <h4 class="fs-20 font-w600  me-auto">CMS EEP</h4>
</div>
<!-- Meta Details -->
...
<div class="row">
            ...
            <div class="card">
                <div class="bg-primary sec-head">
                    ...
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('eep_page', <?php echo $meta_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $meta_data['status'] === '1' ? 'checked' : '' ?>>
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="card">
                    <div class="bg-primary sec-head">
                        ...
                        <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                            <input class="form-check-input" onchange="changeStatus('eep_page', <?php echo $heroData['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $heroData['status'] === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                        </div>
                    </div>
                    <div class="card-body">
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="card">
                    <div class="bg-primary sec-head">
                        ...
                        <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                            <input class="form-check-input" onchange="changeStatus('eep_page', <?php echo $section_two_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $section_two_data['status'] === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                        </div>
                    </div>
                    <div class="card-body">
    ...
    <div class="row">
        <?php
        $seectionThreeLines = $db->db("SELECT * FROM eep_page WHERE main_title IS NULL AND section_name='section_two'");
                ...
                <div class="card">
                    <div class="jobs2 text-center card-body">
                        ...
                        <div class="action-buttons d-flex justify-content-center">
                            <a href="cms-eep.php?id=<?php echo $row['id'] ?>" class="btn btn-danger light">
                            ...
                            <a href="javascript:void(0);" class="btn btn-success light" data-bs-toggle="modal" data-bs-target="#edit_section_two_modal" onclick="edit_section_two('<?php echo $row['title'] ?>',<?php echo $row['id'] ?>)">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1" class="svg-main-icon">
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                        <rect x="0" y="0" width="24" height="24"></rect>
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="card">
                    <div class="bg-primary sec-head">
                        ...
                        <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                            <input class="form-check-input" onchange="changeStatus('eep_page', <?php echo $section_three_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $section_three_data['status'] === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                        </div>
                    </div>
                    <div class="card-body">
    ...
    <div class="row">
        <?php
        $seectionThreeLines = $db->db("SELECT * FROM eep_page WHERE main_title IS NULL AND section_name='section_three'");
        while ($row = mysqli_fetch_assoc($seectionThreeLines)) {
        ?>
            <div class="col-md-3 col-sm-6">
                <div class="card">
                    <div class="jobs2 text-center card-body">
                        ...
                        <div class="action-buttons d-flex justify-content-center">
                            <a href="cms-eep.php?section=three&id=<?php echo $row['id'] ?>" class="btn btn-danger light">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1" class="svg-main-icon">
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                        <rect x="0" y="0" width="24" height="24"></rect>
                                        <path d="M6,8 L6,20.5 C6,21.3284271 6.67157288,22 7.5,22 L16.5,22 C17.3284271,22 18,21.3284271 18,20.5 L18,8 L6,8 Z" fill="#000000" fill-rule="nonzero"></path>
    ...
    <div class="modal fade" id="edit_section_two_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form>
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="label">Title</label>
                            <input type="text" id="section_two_edit_title" class="form-control">
                        </div>
                        <input type="text" id="section_two_edit_id">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary close" data-bs-dismiss="modal">Close</button>
                        <button type="button" class="btn btn-primary update_section_two">Update</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function edit_section_two(title, id){
            $("#section_two_edit_title").val(title);
            $("#section_two_edit_id").val(id);
        }

        $(".update_section_two").click(function() {
            let title = $("#section_two_edit_title").val();
            let id = $("#section_two_edit_id").val();
            $.post("ajax-request.php", {
                    id: id,
                    title: title,
                    page: 'cms-eep',
                    action: 'update-section-two'
                },
                function(data) {
                    console.log(data);
                    if (data == 1) {
