...
$section_meta = $db->db("SELECT * FROM product_reviews_page WHERE section_name = 'meta'");
if (isset($_POST['save_meta'])) {
    ...
    if (mysqli_num_rows($section_meta) > 0) {
        $res = $db->db("UPDATE product_reviews_page SET main_title='$main_title',main_description='$main_description' WHERE section_name='meta'");
        if ($res) {
            alert("Updated", "Meta Updated Successfully", "success");
        }
    } else {
        $sql = "INSERT INTO product_reviews_page (section_name,main_title,main_description)
...
$meta_data = false;
$meta = $db->db("SELECT * FROM product_reviews_page WHERE section_name = 'meta'");
if (mysqli_num_rows($meta) > 0) {
    $meta_data = mysqli_fetch_assoc($meta);
}

//////---------Saving And updating hero section--------------////////////////////////////////////////////////
$carrer = $db->db("SELECT * FROM product_reviews_page WHERE section_name = 'hero'");
$heroTemp = mysqli_fetch_assoc($carrer);
if (isset($_POST['save_hero_section'])) {
    $main_title = filter($_POST['main_title']);
    $main_image = $_POST['main_image'];
    if (mysqli_num_rows($carrer) > 0) {
        $res = $db->db("UPDATE product_reviews_page SET main_title='$main_title',main_image='$main_image' WHERE section_name='hero'");
        if ($res) {
            alert("Updated", "Hero Section Updated Successfully", "success");
        }
    } else {
        $res = $db->db("INSERT INTO product_reviews_page (section_name,main_title,main_image)
...
$section_two = $db->db("SELECT * FROM product_reviews_page WHERE section_name = 'section_two'");
...
if (isset($_POST['save_section_two'])) {
    $iframe = $_POST['iframe'];
    $sql = "INSERT INTO product_reviews_page (section_name,iframe)
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $res = $db->db("DELETE FROM product_reviews_page WHERE id = '$id'");
    if ($res) {
        redirect('cms-product-reviews.php');
    }
}

?>
<div class="d-flex align-items-center mb-4 flex-wrap">
    <h4 class="fs-20 font-w600  me-auto">CMS Product Reviews</h4>
</div>
<!-- Meta Details -->
...
<div class="row">
            ...
            <div class="card">
                <div class="bg-primary sec-head">
                    ...
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('product_reviews_page', <?php echo $meta_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $meta_data['status'] === '1' ? 'checked' : '' ?>>
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="card">
                    <div class="bg-primary sec-head">
                        ...
                        <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                            <input class="form-check-input" onchange="changeStatus('product_reviews_page', <?php echo $careerData['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $careerData['status'] === '1' ? 'checked' : '' ?>>
                            <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                        </div>
                    </div>
                    <div class="card-body">
    ...
    <div class="row">
        <div class="col-xl-12">
            <h4 class="mt-4 text-white p-3 bg-primary sec-head">Product Reviews</h4>
            <form method="post" enctype="multipart/form-data">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-xl-12  col-md-6 mb-4">
                                <label class="form-label font-w600">Video URL<span class="text-danger scale5 ms-2">*</span></label>
                                <input type="text" placeholder="Video URL" required name="iframe" class="form-control solid" aria-label="name">
                            </div>
    ...
    <div class="row">
        <?php
        $seectionThreeLines = $db->db("SELECT * FROM product_reviews_page WHERE main_title IS NULL AND section_name='section_two'");
                ...
                <div class="card">
                    <div class="jobs2 text-center card-body">
                        <div class="text-center">
                            <iframe class="media__video-frame" style="width:100%" src="<?php echo $row['iframe']; ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                        </div>
                        <div class="mb-3">
                            <h6><?php echo $row['title'] ?></h6>
                        </div>
                        <div class="action-buttons d-flex justify-content-center">
                            <a href="cms-product-reviews.php?id=<?php echo $row['id'] ?>" class="btn btn-danger light">
                            ...
                            <div class="form-check form-switch d-flex align-items-center mx-1">
                                <input class="form-check-input" onchange="changeStatus('product_reviews_page', <?php echo $row['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $row['status'] === '1' ? 'checked' : '' ?>>
                            </div>
                        </div>
                    </div>
                </div>
