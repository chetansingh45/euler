...
$section_meta = $db->db("SELECT * FROM term_and_condition_page WHERE section_name = 'meta'");
if(isset($_POST['save_meta'])){
    ...
    if(mysqli_num_rows($section_meta) > 0){
        $res = $db->db("UPDATE term_and_condition_page SET main_title='$main_title',main_description='$main_description' WHERE section_name='meta'");
        if($res){
            alert("Updated","Meta Updated Successfully","success");
        }
    }else{
        $sql = "INSERT INTO term_and_condition_page (section_name,main_title,main_description)
...
$meta_data = false;
$meta = $db->db("SELECT * FROM term_and_condition_page WHERE section_name = 'meta'");
if(mysqli_num_rows($meta) > 0){
    $meta_data = mysqli_fetch_assoc($meta);
}

//////---------Saving And updating hero section--------------////////////////////////////////////////////////
$carrer = $db->db("SELECT * FROM term_and_condition_page WHERE section_name = 'hero'");
...
if(isset($_POST['save_hero_section'])){
    ...
    if(mysqli_num_rows($carrer) > 0){
        $res = $db->db("UPDATE term_and_condition_page SET main_title='$main_title',main_image='$main_image' WHERE section_name='hero'");
        if($res){
            alert("Updated","Hero Section Updated Successfully","success");
        }
    }else{
        $res = $db->db("INSERT INTO term_and_condition_page (section_name,main_title,main_image)
...
$section_two = $db->db("SELECT * FROM term_and_condition_page WHERE section_name = 'section_two'");
...
if(isset($_POST['save_section_two'])){
    $conn = mysqli_connect("localhost", "root", "", "euler");
    $main_description = mysqli_real_escape_string($conn,$_POST['main_description']);
    if(mysqli_num_rows($section_two) > 0){
        $res = $db->db("UPDATE term_and_condition_page SET main_description='$main_description' WHERE section_name='section_two'");
        if($res){
            alert("Updated","Section Two Updated Successfully","success");
        }
    }else{
        $sql = "INSERT INTO term_and_condition_page (section_name,main_description)
...
<div class="d-flex align-items-center mb-4 flex-wrap">
	<h4 class="fs-20 font-w600  me-auto">CMS Refund Policy</h4>
</div>
<!-- Meta Details -->
        ...
        <div class="card">
            <div class="bg-primary sec-head">
                ...
                <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                    <input class="form-check-input" onchange="changeStatus('term_and_condition_page', <?php echo $meta_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $meta_data['status'] === '1' ? 'checked' : '' ?>>
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="card">
                <div class="bg-primary sec-head">
                    ...
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('term_and_condition_page', <?php echo $careerData['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $careerData['status'] === '1' ? 'checked' : '' ?>>
                        <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-xl-6  col-md-6 mb-4">
                            ...
                            <input required type="text" name="main_image" class="form-control solid"  aria-label="name"
                            <?php if($careerData){?>
                             value="<?php echo $careerData['main_image']?>" <?php }?> 
                            >
                        </div>
    ...
    <div class="col-xl-12">
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="bg-primary sec-head">
                    ...
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('term_and_condition_page', <?php echo $section_two_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $section_two_data['status'] === '1' ? 'checked' : '' ?>>
                        <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                    </div>
                </div>
                <div class="card-body">
