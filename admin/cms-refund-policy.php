...
$section_meta = $db->db("SELECT * FROM refund_policy_page WHERE section_name = 'meta'");
if(isset($_POST['save_meta'])){
    ...
    if(mysqli_num_rows($section_meta) > 0){
        $res = $db->db("UPDATE refund_policy_page SET main_title='$main_title',main_description='$main_description' WHERE section_name='meta'");
        if($res){
            alert("Updated","Meta Updated Successfully","success");
        }
    }else{
        $sql = "INSERT INTO refund_policy_page (section_name,main_title,main_description)
...
$carrer = $db->db("SELECT * FROM refund_policy_page WHERE section_name = 'hero'");
$heroTemp = mysqli_fetch_assoc($carrer);
if(isset($_POST['save_hero_section'])){
    $main_title = filter($_POST['main_title']);
    $main_image = $_POST['main_image'];
    if(mysqli_num_rows($carrer) > 0){
        $res = $db->db("UPDATE refund_policy_page SET main_title='$main_title',main_image='$main_image' WHERE section_name='hero'");
        if($res){
            alert("Updated","Hero Section Updated Successfully","success");
        }
    }else{
        $res = $db->db("INSERT INTO refund_policy_page (section_name,main_title,main_image)
...
$section_two = $db->db("SELECT * FROM refund_policy_page WHERE section_name = 'section_two'");
...
if(isset($_POST['save_section_two'])){
    ...
    if(mysqli_num_rows($section_two) > 0){
        $res = $db->db("UPDATE refund_policy_page SET main_description='$main_description' WHERE section_name='section_two'");
        if($res){
            alert("Updated","Section Two Updated Successfully","success");
        }
    }else{
        $sql = "INSERT INTO refund_policy_page (section_name,main_description)
        ...
        <div class="card">
            <div class="bg-primary sec-head">
                    ...
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('refund_policy_page', <?php echo $meta_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $meta_data['status'] === '1' ? 'checked' : '' ?>>
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="card">
                <div class="bg-primary sec-head">
                    ...
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('refund_policy_page', <?php echo $careerData['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $careerData['status'] === '1' ? 'checked' : '' ?>>
                        <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                    </div>
                </div>
                <div class="card-body">
...
<div class="row">
    <div class="col-xl-12">
        <form method="post" enctype="multipart/form-data">
            <div class="card">
                <div class="bg-primary sec-head">
                    <h4 class="text-white p-3 bg-primary d-inline-block">Refund Policy</h4>
                    <div class="form-check form-switch float-end d-inline-block my-3 px-4">
                        <input class="form-check-input" onchange="changeStatus('refund_policy_page', <?php echo $section_two_data['id'] ?>, this)" type="checkbox" role="switch" id="flexSwitchCheckDefault" style="width: 2.5em;" <?php echo $section_two_data['status'] === '1' ? 'checked' : '' ?>>
                        <label class="form-check-label text-white" for="flexSwitchCheckDefault">Hide/Show</label>
                    </div>
                </div>
                <div class="card-body">
