...
if (isset($_GET['id'])) {
    if (isAdmin() || isJrAdmin()) {
        $id = $_GET['id'];
        $res = $db->db("DELETE FROM getupdates WHERE id = '$id'");
        if ($res) {
...
<div class="row">
    <div class="col-12">
        <div class="table-responsive">
            ...
            <a href="newsletter.php?filter=day" class="btn btn-primary mx-1 <?php echo isset($_GET['filter']) && $_GET['filter'] == 'day' ? 'disabled' : '' ?>">Today</a>
            <a href="newsletter.php?filter=week" class="btn btn-primary mx-1 <?php echo isset($_GET['filter']) && $_GET['filter'] == 'week' ? 'disabled' : '' ?>">This Week</></a>
            <a href="newsletter.php?filter=month" class="btn btn-primary mx-1 <?php echo isset($_GET['filter']) && $_GET['filter'] == 'month' ? 'disabled' : '' ?>">This Month</></a>
            <a href="newsletter.php"><button class="btn btn-primary mx-1 <?php echo !isset($_GET['filter']) ? 'disabled' : '' ?>">All</button></a>
...
<script>
    ...
    window.onload = () => {
            ...
            if (isAdmin() || isJrAdmin()) : ?> {
                    ...
                    render: function(data, type, row) {
                        return `<div style="width: 100px" class="text-center">
                            <a href="newsletter.php?id=${row.id}" class="btn btn-sm btn-outline-danger"><i class="fa fa-trash"></i></a>
                        </div>`;
                    }
                }
            <?php endif; ?>
        ...
        initializeDatatable({
            Url: "./php/module/newsletter/list.php?<?php echo $filter; ?>",
            columns
        });
    }
</script>
